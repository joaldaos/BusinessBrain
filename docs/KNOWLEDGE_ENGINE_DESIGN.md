...
**Estado: 🧊 ARQUITECTURA CONGELADA (2026-07-22), con dos revisiones formales aplicadas (2026-07-22 — ver "Revisión formal — Subfase 2.2"; y 2026-07-28 — ver "Revisión formal — Adaptación al Understanding Engine").** Aprobado por el usuario como base del diseño; sometido después a una auditoría externa adversarial (ver "Auditoría externa previa a la congelación") que encontró y corrigió 12 defectos reales antes de esta congelación. Es la especificación oficial de la Fase 2. Cualquier cambio posterior a este estado debe tratarse como una revisión formal del documento (nueva ronda de aprobación), no como una edición silenciosa.

> **Revisión formal — Adaptación al Understanding Engine (2026-07-28).** Alcance estrictamente limitado a cinco puntos, aprobados en bloque por el usuario: **no modifican el modelo de dominio, el pipeline ni ninguna decisión de este documento**, solo declaran la superficie que el Knowledge Engine expone ahora que existe un dominio consumidor sistemático ([`docs/UNDERSTANDING_ENGINE_DESIGN.md`](./UNDERSTANDING_ENGINE_DESIGN.md), 🧊 congelado v1.0). (1) §1.4 declara que las señales operativas no solo se generan sino que se publican como contrato, entregando hechos y nunca veredictos. (2) §3.7 declara el grafo de linaje como consultable desde fuera del dominio. (3) Se añade §13.1 con los tres contratos de metadatos expuestos, incluida la resolución atómica de estado por referencia. (4) §15 aclara que esos contratos no son una excepción al aislamiento porque no entregan contenido de `KnowledgeChunk`. (5) Ajuste cosmético: la sección "Propuestas para Fase 3" pasa a "Propuestas de evolución futura", porque la Fase 3 del roadmap general es ahora el Understanding Engine y esas propuestas son mejoras de este documento. Además, la renumeración de las fases posteriores del roadmap (§10 del plan de migración) se refleja mecánicamente en las referencias de este documento, sin cambio de contenido.
...
- **No orquesta la evolución del producto.** *(Referencia añadida 2026-07-24, sin cambio de dominio.)* El Knowledge Engine genera señales objetivas que son insumo relevante para el Principio de Evolución Asistida (`BUSINESSBRAIN_MIGRATION_PLAN.md` §3.2) — por ejemplo, obsolescencia de conocimiento por decaimiento de confianza (§8.3), conflictos de canonicalización sin resolver (§10), o fuentes desconectadas (§8.2). Generar esas señales es responsabilidad del Knowledge Engine; decidir si constituyen una oportunidad de mejora, justificarla, estimar su impacto y proponerla a la organización es responsabilidad exclusiva de ese principio de producto — el Knowledge Engine no evalúa, no prioriza ni presenta esas propuestas.
- **No interpreta sus propias señales, pero sí las expone.** *(Revisión formal 2026-07-28, adaptación al Understanding Engine.)* Además de generarlas, el Knowledge Engine las **publica como contrato consumible** (§13.1), porque a partir de la Fase 3 existe un dominio que las consume de forma sistemática. La frontera se mantiene intacta: el Knowledge Engine entrega **hechos** (qué decayó, qué conflicto sigue sin resolver, qué fuente está desconectada, qué versión tiene hoy una evidencia) y **nunca veredictos** sobre lo que esos hechos significan. Interpretar si un cambio invalida un razonamiento es epistemología y pertenece exclusivamente al Understanding Engine — si el Knowledge Engine emitiera ese juicio, necesitaría conocer entidades de un dominio superior, invirtiendo la dependencia.
...
**Toda decisión se evalúa por su aporte a la comprensión del negocio.** Principio arquitectónico permanente, no exclusivo del Knowledge Engine (añadido en la Revisión formal — Subfase 2.2): cualquier decisión de diseño o de implementación, en cualquier módulo de BusinessBrain, se justifica por su capacidad de mejorar lo que BusinessBrain entiende de la empresa — qué sabe, cuánto puede confiarse en ello, de dónde viene, si sigue vigente. Una solución técnicamente elegante, eficiente o interesante que no mejora esa comprensión no debe implementarse solo porque sea posible construirla. Este principio se aplica por encima de cualquier preferencia de implementación individual y debe usarse como criterio de corte ante cualquier propuesta de mejora futura (ver "Propuestas de evolución futura").

**La canonicalización es completamente independiente del algoritmo que la consuma.** Principio arquitectónico permanente (añadido en la auditoría de implementación de la Subfase 2.2, hallazgo F, al aprobarse el contrato de contenido canónico — ver §3.12): ningún mecanismo, presente o futuro, podrá definir su propia canonicalización. Todos deberán utilizar obligatoriamente la misma representación canónica del contenido. La canonicalización en sí (qué transformaciones aplica, qué preserva) es un concepto de dominio independiente de cualquier algoritmo concreto que la consuma — hash exacto (§7, nivel 1), similitud estructural (§7, nivel 2), o cualquier mecanismo de identidad de contenido que se añada en el futuro (nivel 3 operativo, fingerprinting, embeddings). Este principio existe porque su ausencia ya produjo un defecto real: dos mecanismos que interpretaban el mismo documento de forma distinta generaban resultados de deduplicación contradictorios entre sí.
...
> **Consultable desde fuera del dominio** *(Revisión formal 2026-07-28)*. Este grafo deja de ser estructura exclusivamente interna: se expone como contrato de consulta y recorrido (§13.1) para que un dominio consumidor pueda analizar relaciones históricas o estructurales entre `KnowledgeItem` — frecuencia de actualización de un documento, fusiones recientes en una colección — sin acceder a `KnowledgeChunk` ni al almacén vectorial. Toda consulta va acotada por organización; nunca se expone una exportación completa del grafo.
...
**Nota de alcance para la Fase 2 (hallazgo #7 de la auditoría previa a la congelación; corregida en la Revisión formal — Subfase 2.2, hallazgo B).** La formulación original de esta nota razonaba por *tipo de conector* ("mientras el único conector sea la carga manual") para justificar que el nivel 3 no tiene candidatos reales en Fase 2. Eso era impreciso: la frontera del nivel 3, fijada arriba, es por **instancia de `KnowledgeSource`**, no por tipo de conector — una organización puede crear varias `KnowledgeSource` de tipo carga manual (p. ej. "Subidas RR. HH." y "Subidas Operaciones") y subir a ambas contenido que describa el mismo hecho, lo cual **sí** es un candidato legítimo de nivel 3 desde la propia Fase 2, sin esperar a la Fase 7.
...
### 13.1 Superficie de metadatos expuesta a dominios consumidores

*(Revisión formal 2026-07-28, adaptación al Understanding Engine. No modifica el modelo de dominio ni el pipeline anterior.)*

El Retriever sigue siendo **el único camino de acceso a `KnowledgeChunk` y al almacén vectorial**, sin excepción (§15). Los tres contratos que siguen no lo contradicen porque **no entregan contenido**: exponen metadatos sobre entidades del Knowledge Engine —estado, estructura y pertenencia— que un dominio consumidor necesita para razonar *sobre* el conocimiento sin recuperarlo. Todos van acotados por organización de forma no negociable, igual que el Retriever.

- **Señales operativas.** Las ya descritas en §1.4: decaimiento de confianza por debajo de umbral (§8.3), conflictos de canonicalización sin resolver (§10) y fuentes desconectadas (§3.2, §5). Se entregan como conjunto de señales tipadas con referencia a la entidad que las originó y su fecha — eventos o estado resumido, nunca estructura de grafo.
- **Consulta del grafo de linaje.** Recorrido acotado de las relaciones tipadas entre `KnowledgeItem` (§3.7). Contrato deliberadamente separado del anterior: una lista plana de señales y una estructura de grafo son responsabilidades distintas, y fusionarlas acoplaría dos formas de datos muy diferentes bajo un mismo contrato.
- **Resolución de estado por referencia.** Dado un conjunto de referencias a evidencias ya citadas, devuelve **en una sola operación** su pertenencia actual a `KnowledgeCollection`, su versión, su estado de ciclo de vida y los metadatos necesarios. Dos exigencias son parte inseparable de este contrato:
  - **Devuelve hechos, nunca veredictos.** Versión, estado, pertenencia — jamás conclusiones del tipo "esta evidencia invalida ese razonamiento". Qué cambio invalida qué comprensión depende del razonamiento y no del documento: es epistemología, y pertenece al dominio consumidor (§1.4).
  - **La atomicidad es obligación del proveedor.** La resolución debe servirse contra un estado único y coherente. Un consumidor no puede garantizar unilateralmente la atomicidad de una operación que ejecuta este dominio; componerla a partir de lecturas independientes en instantes distintos permitiría obtener un resultado que no existió en ningún momento real, con consecuencias de seguridad para quien lo use como base de una decisión de acceso.
...
**Aislamiento entre organizaciones.** El filtro por organización en la recuperación (§13, paso 3) se trata como un requisito estructural, no como una condición de negocio más: toda ruta de acceso a `KnowledgeChunk` pasa obligatoriamente por el Retriever, y el Retriever nunca ejecuta una búsqueda sin ese filtro. No existe, por diseño, ningún camino alternativo de lectura directa del almacén vectorial que lo bypasee. Los contratos de metadatos expuestos a dominios consumidores (§13.1) **no constituyen una excepción**: no entregan contenido de `KnowledgeChunk` y están sujetos al mismo filtro de organización obligatorio.
...
8. **Puerta de salida de Fase 2**: solo cuando 2.1–2.7 están validadas de forma independiente se autoriza iniciar la Fase 3 (Understanding Engine como primer consumidor del Retriever), tal como ya establece el roadmap general del plan de migración.
...
- **Conectores más allá de carga manual de archivos.** Google Drive, Gmail, CRM, ERP, web scraping y bases de datos externas quedan definidos como Connectors conceptuales en este documento, pero su implementación real es de la Fase 7 (integraciones) del plan de migración general.
- **Marketplace o catálogo de plantillas de agentes.** No es responsabilidad del Knowledge Engine y no se toca en esta fase.
- **Hardening de aislamiento multi-tenant a nivel de infraestructura** (políticas de seguridad a nivel de base de datos como segunda capa de defensa, rate limiting, pruebas de carga): corresponde a la fase de hardening (Fase 10) del roadmap general; en Fase 2 el aislamiento se garantiza a nivel de diseño de dominio y se verifica con pruebas de integración, no con esa segunda capa de defensa todavía.
...
- **Límites de uso por plan de suscripción** (cuotas de documentos indexados, tokens de embeddings por mes): corresponden a la Fase 8 (billing) del plan de migración general.
- **Resolución automática de conflictos de canonicalización con datos reales entre fuentes distintas** (añadido tras la auditoría previa a la congelación, hallazgo #7; corregido en la Revisión formal — Subfase 2.2, hallazgo B): el mecanismo se modela y se construye en Fase 2, pero el nivel 3 de deduplicación que lo alimenta (§7) solo tiene lógica de comparación real a partir de que exista capacidad de embeddings a nivel de documento (subfase 2.6 en adelante) — no a partir de que exista más de un tipo de conector (Fase 7), que era la formulación original de este punto. Un conflicto genuino puede darse ya en Fase 2 con varias `KnowledgeSource` de carga manual con contenido solapado, pero no se detecta automáticamente hasta que el nivel 3 esté activo. Su validación en Fase 2 (subfases 2.2-2.5) es sintética; su primera validación orgánica es responsabilidad de la primera subfase donde el nivel 3 tenga lógica real, no necesariamente de la Fase 7.
...
- **Interfaz de usuario.** Este documento y su implementación son de backend/dominio; cualquier pantalla para explorar conocimiento, revisar conflictos de canonicalización o ajustar confianza manualmente es responsabilidad de la Fase 9 (frontend) del plan general.
...
| B | Contradicción | §7 ("Nota de alcance para la Fase 2") y §10/§20 justificaban la desactivación del nivel 3 de deduplicación razonando por *tipo de conector* ("hasta que exista más de un tipo de KnowledgeSource"/"más de un conector"), mientras que la frontera real del nivel 3 (§7, "Frontera con el linaje de versiones") se define por *instancia* de `KnowledgeSource`. Con el único conector de Fase 2 (carga manual), una organización puede tener ya hoy varias `KnowledgeSource` con contenido solapado — un candidato de nivel 3 legítimo, no hipotético hasta la Fase 7. | Se corrige la justificación en §7, §10 y §20: el nivel 3 puede tener candidatos reales desde la propia Fase 2; lo que realmente lo mantiene inactivo es la falta de dos capacidades del roadmap (embeddings a nivel de documento, subfase 2.6; `Canonical Knowledge Entity`, subfase 2.5), no la ausencia de otro tipo de conector. |
