...
El chat, los agentes, las automatizaciones y los informes son **consumidores del mismo Knowledge Engine** a través de un único caso de uso de recuperación (`retrieve-context`); ninguno tiene acceso privilegiado o un camino de datos distinto a los demás.
...
| `ConversationsModule` *(superficie de consumo, no el núcleo)* | Interfaz de chat sobre el Knowledge Engine y los Agentes | `GET/POST /conversations`, `GET /conversations/:id`, `POST /conversations/:id/messages`, streaming vía WS/SSE | Delega la recuperación de contexto en `retrieve-context.use-case` de `knowledge-engine`; no contiene lógica de RAG propia. |
...
Todas llaman al mismo `retrieve-context.use-case`:
...
| **3** | ⬜ Pendiente | `ConversationsModule` como **primer consumidor** de `retrieve-context.use-case` | Chat funcional con respuestas contextualizadas y citas, sin lógica de RAG propia |
| **4** | ⬜ Pendiente | `AgentsModule` (capacidades/tools/memoria/permisos/guardrails) + `RecommendationsModule`; se crea el modelo `AgentTemplate` (sin UI de marketplace todavía) | Agentes por área operativos; catálogo de plantillas listo para exponer más adelante sin migración |
| **5** | ⬜ Pendiente | `AutomationsModule` + `ReportsModule` | Flujos programados e informes exportables (PDF) |
| **6** | ⬜ Pendiente | `IntegrationsModule` (CRM/ERP/Google Drive/Gmail/Slack…) como conectores reales del `KnowledgeEngineModule` | Conectores externos reales, más allá de subida manual |
| **7** | ⬜ Pendiente | `BillingModule` + `ApiKeysModule` + límites de plan | Monetización y acceso programático |
| **8** | ⬜ Pendiente | Frontend completo contra la nueva API (TypeScript) | UI de producto reemplaza landing/demo actual |
| **9** | ⬜ Pendiente | Hardening: RLS, rate limiting, observabilidad, pruebas de carga, auditoría de aislamiento entre tenants sobre `retrieve-context.use-case` | Listo para producción multi-cliente |
