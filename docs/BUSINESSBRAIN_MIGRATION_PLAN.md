...
| `UnderstandingEngineModule` ★ *(v1.2, núcleo)* | **Bounded context de la comprensión**: ejecuciones de razonamiento, `Insight` con evidencia trazable y confianza compuesta, objetivos de negocio y gate de riesgo/oportunidad, curación humana y puente con `Recommendation`. Consume el Knowledge Engine solo por contratos declarados, nunca `KnowledgeChunk` directo. Especificado en [`docs/UNDERSTANDING_ENGINE_DESIGN.md`](./UNDERSTANDING_ENGINE_DESIGN.md) (🧊 congelado v1.0) | Sin HTTP público en la Fase 3: `RetrieveInsights` se valida como capacidad interna, sin consumidores conectados | Único punto de lectura de comprensión para toda superficie futura. Nunca ejecuta acciones: cualquier acción pasa por el Principio de Evolución Asistida (§3.2). |
...
| **2** | ⬜ Pendiente | `KnowledgeEngineModule` completo: `KnowledgeSource` + `IngestionJob` + normalización + clasificación + dedup + versionado + confianza + `KnowledgeChunk`/pgvector | Ingesta de conocimiento end-to-end, con historial de versiones y sin duplicados — **antes de construir el chat** |
| **3** | ⬜ Pendiente | **`UnderstandingEngineModule`** — especificado en [`docs/UNDERSTANDING_ENGINE_DESIGN.md`](./UNDERSTANDING_ENGINE_DESIGN.md) (🧊 congelado v1.0): `AnalysisRun` + `Insight` (con identidad de sujeto, cierre transitivo de evidencia y proyecciones vivas de alcance y frescura) + `BusinessObjective` y gate de riesgo/oportunidad + razonamiento generativo + confianza compuesta + curación humana + `RetrieveInsights` | Comprensión derivada, justificada y trazable sobre el conocimiento indexado — **antes de construir el chat**. Sin consumidores conectados todavía |
| **4** | ⬜ Pendiente | `ConversationsModule` y demás superficies conversacionales, como **primeros consumidores del Understanding Engine** (`RetrieveInsights`), además del Retriever del Knowledge Engine | Chat funcional con respuestas contextualizadas y citas, apoyado en comprensión y no solo en recuperación; sin lógica de RAG ni de razonamiento propia |
| **5** | ⬜ Pendiente | `AgentsModule` (capacidades/tools/memoria/permisos/guardrails) + `RecommendationsModule`; se crea el modelo `AgentTemplate` (sin UI de marketplace todavía) | Agentes por área operativos; catálogo de plantillas listo para exponer más adelante sin migración |
| **6** | ⬜ Pendiente | `AutomationsModule` + `ReportsModule` | Flujos programados e informes exportables (PDF) |
| **7** | ⬜ Pendiente | `IntegrationsModule` (CRM/ERP/Google Drive/Gmail/Slack…) como conectores reales del `KnowledgeEngineModule` | Conectores externos reales, más allá de subida manual |
