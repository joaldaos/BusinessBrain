...
2. `POST /knowledge-sources/:knowledgeSourceId/sync` con un archivo adjunto (`multipart/form-data`, campo `file`) dispara `IngestFromSourceUseCase.execute()`:
   ...
   - por cada resultado extraído, `normalizeContent()` (`application/normalize-content.use-case.ts`) convierte el contenido crudo a texto normalizado (`contentText`, lo que se almacena) y calcula su hash con `computeContentHash()` (`domain/content-canonicalization.ts`) — nunca sobre `contentText` directamente, siempre sobre su forma canónica (§3.12);
...
- **Contenido canónico como contrato único de identidad** (KNOWLEDGE_ENGINE_DESIGN.md §3.12, `domain/content-canonicalization.ts`). Nivel 1 (`computeContentHash`) y nivel 2 (`computeShingles`) operan siempre sobre `canonicalizeContent(contentText)`, nunca sobre el texto crudo cada uno por su cuenta — corrige un defecto real encontrado en la auditoría de implementación (nivel 1 y nivel 2 normalizaban el espacio en blanco de forma distinta entre sí, produciendo versiones espurias ante reingestas sin cambio de contenido real). Incluye plegado de mayúsculas/minúsculas. Un test de contrato (`content-canonicalization.contract.spec.ts`) rompe en CI si nivel 1 y nivel 2 llegan a divergir de nuevo. **No confundir con "canonicalización" en el sentido de §10** (`Canonical Knowledge Entity`, resolución de conflictos entre fuentes distintas, subfase 2.5) — son dos conceptos distintos que comparten palabra pero no dominio: uno decide "¿es el mismo texto?" (identidad de contenido), el otro decide "¿cuál de varias fuentes prevalece?" (resolución de conflicto).
- **Migración de datos, no de esquema, tras un cambio de canonicalización** (`scripts/recompute-knowledge-item-content-hashes.ts`, `npm run backfill:content-hashes` en `apps/backend`). Recalcula `contentHash` de todo `KnowledgeItem` existente (cualquier estado) para que quede comparable con el código actual; idempotente; detecta pero no resuelve automáticamente colisiones que el recálculo pueda revelar entre `KnowledgeItem` activos — quedan para revisión humana.
