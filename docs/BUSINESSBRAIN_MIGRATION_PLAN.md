...
| **7** | 🔄 En curso | `IntegrationsModule`. **Página web sin OAuth** (`web_page_v1`, guard anti-SSRF sobre la IP resuelta y cada salto de redirección), **Google Drive con OAuth** (`google_drive_v1`, estado firmado atado a cookie `HttpOnly`, tokens cifrados que nunca salen, sincronización incremental por marcador, revocación que detiene las fuentes) y **Gmail** (`gmail_v1`, ver la fila siguiente). Todos `PULL` y programables con `SYNC_KNOWLEDGE_SOURCE`. Pendientes: adjuntos y el resto de conectores (CRM, ERP, redes, mensajería) | Conectores externos reales, más allá de subida manual |
| **7.1** | ✅ Completada | **Gmail** (`gmail_v1`): un mensaje = un `KnowledgeItem`, sin entidad de hilo; **etiqueta como frontera** de sincronización, comprobada por duplicado porque `gmail.readonly` alcanza al buzón entero; **perímetro de colección restringida exigido estructuralmente** al crear la fuente **y en cada sincronización** (`RestrictedPerimeterService`); la dirección del remitente queda fuera del conocimiento recuperable y vive en `KnowledgeItem.sourceMetadata`; adjuntos fuera de V1; permiso mínimo `gmail.readonly`; estado de OAuth atado también al PROVEEDOR. Se corrigen además tres fallos que la fase deja al descubierto: `GET /knowledge-items` no acotaba por colección ni ocultaba la metadata operativa, y la cookie del flujo OAuth era `SameSite=Strict` — con lo que **ninguna** integración de Google podía conectarse en un navegador real | **Cumplido**: el correo de una persona se convierte en conocimiento de empresa solo dentro del perímetro que alguien decidió. Verificado por HTTP y con navegador real contra un Google sustituido. |
| **7.5** | ✅ Completada | **Los primeros 10 minutos de una PYME.** El recorrido estaba roto en el minuto uno: la interfaz remitía a la API para crear la organización, y el chat con citas —lo único que una PYME entiende en cinco segundos— existía completo en el backend y no se llamaba desde ninguna pantalla. Se añaden: alta de empresa desde la interfaz, pantalla **Preguntar** con fuentes y comprensión utilizada, primeros pasos guiados con estado real, e invitar por enlace (sin envío de correo). Se corrige además el fallo que lo invalidaba todo: **`ChunkAndEmbedUseCase` no lo invocaba nadie**, así que ningún documento subido por una persona real era recuperable — la recuperación es vectorial y no se escribía un solo fragmento. Y se generaliza `externalEndpoint()`, que permite verificar en un navegador real cualquier integración externa sin credenciales | **Cumplido**: una PYME se registra, da de alta su empresa, sube un documento y obtiene una respuesta citando sus propias fuentes. Verificado sin sembrar datos, por HTTP y con navegador. |
| **7.6** | ✅ Completada | **MVP comercial, bloque A — configurar la IA desde la interfaz.** No existía ninguna ruta para `LlmProfile`: la única forma de configurar el proveedor era escribir en la base de datos, así que una instalación nueva nacía muerta —sin vectorizar, nada de lo que sube una empresa se puede preguntar—. Se añade `/ai-configuration` (leer: cualquier miembro; cambiar: ADMIN), con clave cifrada en reposo, **comprobada contra el proveedor antes de guardarla** pidiendo un embedding real, y que no vuelve a salir jamás. Se corrigen dos fallos estructurales que salieron a la luz: seis consumidores pasaban `apiKeyEnc` —el texto CIFRADO— como si fuera la clave y nadie la desciframos nunca; y quien vectorizaba leía el perfil de la organización y llamaba SIEMPRE a OpenAI con esa clave, fuera del proveedor que fuera | **Cumplido**: una empresa nueva deja BusinessBrain operativo sin tocar la base de datos, y sabe si usa su clave o la del servicio. |
| **7.7** | ✅ Completada | **MVP comercial, bloque B — PDF y Word.** El selector ofrecía `.pdf` y `.docx` y la normalización solo aceptaba texto, Markdown y HTML: los documentos habituales de una PYME se rechazaban. Ahora entran por la MISMA tubería (una sola normalización, que sirve también a Drive y Gmail), con reconocimiento del formato REAL por firma de bytes —no por el nombre—, tope de fichero y de texto extraído, y sin ejecutar macros ni contenido activo. **La página del PDF se conserva reutilizando los encabezados que el troceado ya entiende**, así que la cita dice de qué página sale la respuesta sin modelo paralelo. Un PDF escaneado, uno corrupto o uno renombrado fallan individualmente con un mensaje que una PYME entiende, sin tumbar la ingesta | **Cumplido**: una empresa sube su contrato en PDF y su propuesta en Word y puede preguntarlos con citas. Verificado con ficheros reales generados en las pruebas, no sembrados. |
| **7.8** | ✅ Completada | **MVP comercial, bloque C — recomendaciones.** El análisis pasa de comprender a **proponer**: crea `Recommendation` en estado `NEW` a partir de conclusiones vivas, reutilizando la entidad y los ocho campos del contrato que ya existían — sin entidad, estado ni pipeline nuevos. Dos puertas cierran hacia fuera: la regla de elegibilidad (estado, tipo, evidencia, confianza y alcance) y la exigencia de contrato COMPLETO; sin material, cero propuestas. Proponer no aprueba ni ejecuta: la decisión humana es aceptar o descartar, con actor y fecha. El camino manual de escalado, con su curación previa, queda intacto; se distinguen por `createdById` —único campo añadido— porque una PYME debe saber si detrás hay una persona. Pantalla de Recomendaciones con evidencia navegable hasta la conclusión | **Cumplido**: registrarse → configurar IA → subir PDF/Word → preguntar → analizar → conclusión → recomendación → evidencia → aceptar, todo desde la interfaz y verificado en navegador sobre el build real. |
