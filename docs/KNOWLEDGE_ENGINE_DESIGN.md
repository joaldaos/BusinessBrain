...
**Idempotencia de la ingesta.** Volver a sincronizar la misma fuente sin cambios no debe duplicar conocimiento, no debe generar nuevas versiones espurias, y no debe recalcular embeddings innecesariamente (ver §7 y §12).
...
**Relaciones.** Pertenece a una organización. Contiene muchos `KnowledgeItem`; un mismo `KnowledgeItem` puede pertenecer a más de una colección (p. ej. una política de vacaciones puede estar en la colección de RR. HH. y en la de Operaciones). Es referenciada por agentes, automatizaciones e informes como alcance de conocimiento permitido.
...
**Nota de alcance para la Fase 2 (hallazgo #7 de la auditoría previa a la congelación).** El nivel 3 solo tiene candidatos reales que evaluar cuando existe más de un tipo de `KnowledgeSource` activo en la organización. Mientras el único conector disponible sea la carga manual de archivos (§19, subfase 2.1), este nivel queda estructuralmente presente pero no encuentra coincidencias entre fuentes por construcción — no se invierte esfuerzo de implementación en su activación real hasta la Fase 6 (conectores adicionales).
...
| Modelar el nivel 3 de deduplicación semántica y la canonicalización completos desde ahora, pero posponer su activación real hasta que exista más de un tipo de `KnowledgeSource` (§7, §10, §19) — decisión añadida en la auditoría previa a la congelación (hallazgo #7) | Construir e implementar por completo ambos mecanismos ya en Fase 2 | En Fase 2 el único conector es carga manual de archivos: no puede existir todavía un conflicto real entre fuentes distintas. Implementar la resolución automática de conflictos ahora sería coste sin beneficio verificable; modelarlo ahora y activarlo en Fase 6 evita, a la vez, sobreingeniería prematura y una migración de dominio disruptiva más adelante |
...
2. **2.2 — Deduplicación y versionado**: los tres niveles de deduplicación (§7) y el grafo de linaje con sus escenarios (§6). *Validación:* resubir el mismo archivo no duplica; subir una versión modificada crea una nueva versión enlazada; los escenarios de división/fusión pueden probarse manualmente.
...
5. **2.5 — Canonicalización**: agrupación en `Canonical Knowledge Entity`, resolución automática de ganador claro, cola de conflictos para revisión humana. Dado que en Fase 2 solo existe el conector de carga manual (una única `KnowledgeSource` por definición práctica), esta subfase construye el mecanismo completo pero **no tiene datos reales con los que ejercitar un conflicto entre fuentes distintas** (hallazgo #7 de la auditoría previa a la congelación); su validación en esta fase es necesariamente sintética (casos de prueba construidos a mano), no orgánica. La validación con datos reales de conflicto entre fuentes queda pendiente de la Fase 6. *Validación (Fase 2):* dos `KnowledgeItem` de prueba vinculados manualmente como candidatos se agrupan y se resuelven (o se marcan en conflicto) según las reglas de §10.
...
- **Resolución automática de conflictos de canonicalización con datos reales entre fuentes distintas** (añadido tras la auditoría previa a la congelación, hallazgo #7): el mecanismo se modela y se construye en Fase 2, pero no hay más de una `KnowledgeSource` real con la que pueda producirse un conflicto genuino hasta la Fase 6. Su validación en Fase 2 es sintética; su primera validación orgánica es responsabilidad de la Fase 6.
