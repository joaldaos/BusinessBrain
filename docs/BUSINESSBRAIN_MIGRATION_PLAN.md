...
| **7** | 🔄 En curso | `IntegrationsModule`. **Página web sin OAuth** (`web_page_v1`, guard anti-SSRF sobre la IP resuelta y cada salto de redirección) y **Google Drive con OAuth** (`google_drive_v1`, estado firmado atado a cookie `HttpOnly`, tokens cifrados que nunca salen, sincronización incremental por marcador, revocación que detiene las fuentes). Ambos `PULL` y programables con `SYNC_KNOWLEDGE_SOURCE`. Pendientes: Gmail y el resto de conectores | Conectores externos reales, más allá de subida manual |
